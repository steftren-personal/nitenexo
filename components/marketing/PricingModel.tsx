...
/**
 * How pricing works — two panels instead of fixed packages, because every
 * assistant is quoted per project: the honest range for the one-off setup and
 * what drives it, plus the ongoing retainer for support, bugfixes and further
...
export function PricingModel({ polarity = "dark" }: { polarity?: "dark" | "light" }) {
  ...
  const panel = (
    ...
    return (
      ...
      >
        ...
        <div style={{ font: "var(--type-display-large)", fontSize: "clamp(28px, 3.6vw, 44px)", lineHeight: 1.1 }}>{data.amount}</div>
  ...
  return (
    <div>
      <div className="bw-pricing-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "var(--space-lg)", alignItems: "stretch" }}>
        {panel("setup", PRICING.setup)}
