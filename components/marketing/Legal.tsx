...
export function LegalLayout({
  ...
  return (
    <>
      ...
      <div style={{ background: "var(--surface-canvas-light)", color: "var(--ink)", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        <div style={{ flex: 1, maxWidth: 820, width: "100%", margin: "0 auto", padding: "var(--space-xxl) var(--space-xl) var(--space-section)" }}>
          <Eyebrow polarity="light">{eyebrow}</Eyebrow>
          <h1 style={{ font: "var(--type-heading-xl)", fontSize: "clamp(30px, 4.4vw, 44px)", margin: "var(--space-md) 0 var(--space-md)" }}>{title}</h1>
          {intro && <p style={{ font: "var(--type-body-md)", color: "var(--color-accent-violet-mid)", margin: "0 0 var(--space-sm)", maxWidth: 620 }}>{intro}</p>}
          {updated && <p style={{ font: "var(--type-caption)", color: "var(--color-accent-violet-mid)", margin: 0 }}>Stand: {updated}</p>}
...
export function LegalSection({ title, children }: { title?: string; children: React.ReactNode }) {
  return (
    <section>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-md)", font: "var(--type-body-md)", lineHeight: 1.7, color: "var(--ink)" }}>
        {children}
      </div>
    </section>
  );
...
export const legalLink: React.CSSProperties = { color: "var(--color-accent-violet)" };
